...
export class P2ptrackerComponent implements OnInit {
  ...
  constructor(private router: Router,public CS:CommonService,private objservice:P2PtrackerService,public http:HttpClient,private ngxService: NgxUiLoaderService) { }
  public userdtl;
  public user;
...
POfileupload(files,i,data){
  ...
  approveorder(i){
    ...
    let Indata={
      ...
      "finance_approver" : this.finance_approver,
      "prf_number" : this.objporeq.prf_number,
    }
    ...
    recieveddata  => {
     ...
     this.p2prespose=tempresponsedata;
    
      
    });
  this.ngxService.stop(); 
  
    if(this.p2prespose.Status=="Success"){
     
      if(i==1){
        alert("PRF no:"+this.objporeq.prf_number+" Sent to Finance Approve Sucessfully");
        this.list();
      }
     else if(i==2){
