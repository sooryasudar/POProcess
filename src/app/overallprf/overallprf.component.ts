...
declare const $: any;
@Component({
  selector: 'app-overallprf',
  templateUrl: './overallprf.component.html',
  styleUrls: ['./overallprf.component.css']
})
export class OverallprfComponent implements OnInit {
  ...
  approveorder(i){
    ...
    this.financeresponse=tempresponsedata;
      
    });
    if(this.financeresponse.Status=="Success"){
      if(i==1){
...
this.getporequirementlist();
  ...
  getporequirementlist(){ 
    ...
    this.objservice.getoverallFromServer(Indata)
    .subscribe(
    recieveddata  => {
      let tempresponsedata = recieveddata;
      console.log("porequirementlist---",tempresponsedata)
